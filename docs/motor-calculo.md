...
| `Período` | Mes de liquidación/pago del concepto (se normaliza al día 1). Es el mes al que se suma el concepto, salvo que traiga imputación |
| `Imputación` (opcional) | Mes de devengamiento al que se imputa el concepto: se detecta cualquier encabezado que contenga "imputación", y acepta fecha, MM/AAAA, AAAA-MM, AAAAMM o DD/MM/AAAA. Si la celda es una fórmula (p.ej. `=C2`, que copia el `Período`, como en el export de GASA), se toma el resultado que guardó Excel; lo mismo vale para cualquier otra columna. Si la celda no está vacía, el concepto suma al mes de imputación en vez del `Período`: un retroactivo pagado en junio e imputado a abril integra la remuneración **devengada** de abril (art. 245: "mejor remuneración mensual, normal y habitual devengada"; arts. 121-122 para la base del SAC) y deja de inflar junio. El renglón queda en el `detalle` del mes de imputación con `liquidadoEn` = mes de pago, para trazabilidad |
...
| `Proceso` | Nombre del proceso de liquidación. Si contiene "ajuste" (retroactivo) **y el renglón no trae imputación**, el mes se marca `esNormalYHabitual=false`: no se sabe a qué mes corresponde el retroactivo, así que ese mes no compite por la MRMNH. Con imputación no hace falta, porque el ajuste ya está en su mes de devengamiento |
...
`Empleado`, `Modelo`, `Depto.`, `Tipo` y `Contrato` no se usan (no tienen un campo equivalente en el modelo hoy). El resto de columnas (`Doc`, `Apellido y Nombre`, `Período`, `Ingreso`, `Categoría`, `Proceso`, `Código`, `Concepto`, `Monto`, `TIPO`) son obligatorias — el import rechaza el archivo si falta alguna (la de imputación es opcional). La importación es **idempotente**: reimportar el mismo archivo actualiza (no duplica) los mismos `Empleado`/`RemuneracionMensual` vía upsert por `[clienteId, cuil]` / `[empleadoId, periodo]`.

Como el upsert **reemplaza** el mes entero, un mes que en el archivo aparece solo por ajustes imputados desde otros meses de pago (su propia liquidación no está en el archivo, p.ej. un retroactivo imputado a un mes anterior al rango exportado) **no se graba**. Si se grabara, pisaría con el ajuste solo un mes completo importado antes, o crearía un mes que no representa la remuneración real. Esos casos vuelven en `advertencias` del resumen del import, para reimportar un archivo que incluya ese mes.
...
- **`RemuneracionMensual`** del período del recibo — **solo si ese mes no estaba cargado** (por la nómina, §2.2, o a mano) **y el recibo trae base**. El recibo es un único proceso de liquidación; la nómina trae el mes completo. Un recibo de despido que no incluye el sueldo del mes, porque se liquidó en el proceso mensual, dejaba la base en $0 y con ella la indemnización y el preaviso (caso real: DALLOVERE). Por eso un mes ya cargado se mantiene, y si no hay mes y el recibo no trae ningún concepto de la base, no se crea: la auditoría pide la nómina en vez de calcular sobre cero. La respuesta informa cuál de los tres casos se dio (`remuneracionMensual.registrada`: `recibo` / `ya_existia` / `sin_base`). Cuando se carga desde el recibo, `conceptosRemunerativos` se calcula sumando, código por código, los conceptos del recibo cuyo `ConceptoCliente.baseIndemnizacion` es `true` (§2.4) — si el cliente todavía no importó su catálogo, se cae al total "Remunerativo" que imprime el recibo (menos preciso: puede incluir conceptos remunerativos que la doctrina excluye de la base del art. 245, como el SAC proporcional). La respuesta del import (`baseCalculadaConCatalogo`) indica cuál de los dos se usó.
...
- **`Liquidacion` de origen "empresa"**: se cargan los rubros que el recibo sí trae, mapeados por **palabras clave en el nombre del concepto** (no por código — los códigos son específicos del proveedor de nómina, no un estándar): "sac" + "proporcional" → `SAC_PROP`; "vac" + "no goz" (sin "anterior") → `VAC_NO_GOZADAS`; "vac" + "no goz" + "anterior" → `VAC_NO_GOZADAS_ANTERIORES`; con "sac" antepuesto, los mismos dos casos → `SAC_S_VAC` / `SAC_S_VAC_ANTERIORES`; "preaviso" → `PREAVISO` e "integ" → `INTEGRACION_MES`, **con o sin "sac"**, porque el motor calcula esos dos rubros con su SAC incluido (§4.2-4.3) y "SAC sobre Preaviso" / "SAC sobre Integ. del mes" se suman al mismo rubro; "indemn" + "antig" → `IND_ANTIGUEDAD` (sin "indemn" es el adicional mensual por antigüedad, no la indemnización). Los conceptos que caen en el mismo rubro se declaran sumados, un renglón por rubro. La indemnización por fallecimiento (art. 248) queda sin mapear a propósito, porque el motor no la calcula. Si el recibo no incluye la indemnización por antigüedad, el preaviso o la integración del mes (p.ej. porque se liquidan por otro instrumento), quedan declarados en $0: es justamente la señal que la auditoría necesita marcar si esos rubros se pagaron por fuera del recibo.
