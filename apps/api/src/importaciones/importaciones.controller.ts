...
export class ImportacionesController {
  ...
  importarConceptos(@Param('clienteId') clienteId: string, @UploadedFile() archivo?: Express.Multer.File) {
    if (!archivo) throw new BadRequestException('Falta el archivo a importar (campo "archivo")');
    return this.service.importarConceptos(clienteId, archivo.buffer);
  }
