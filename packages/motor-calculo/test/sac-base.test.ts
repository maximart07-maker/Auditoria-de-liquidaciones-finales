...
import { calcularBaseSAC, RemuneracionMensualParaSAC, SinRemuneracionesSemestreError } from '../src/sac-base';
import { utc } from './fixtures';

function remuneracion(overrides: Partial<RemuneracionMensualParaSAC>): RemuneracionMensualParaSAC {
  return { periodo: utc(2024, 1, 1), remuneracionDevengadaSac: 100_000, esNormalYHabitual: true, ...overrides };
}
...
describe('calcularBaseSAC', () => {
  it('toma la mejor remuneración dentro del semestre del egreso, ignorando el otro semestre', () => {
    const remuneraciones = [
      remuneracion({ periodo: utc(2023, 11, 1), remuneracionDevengadaSac: 999_999 }), // semestre anterior, no cuenta
      remuneracion({ periodo: utc(2024, 1, 1), remuneracionDevengadaSac: 100_000 }),
      remuneracion({ periodo: utc(2024, 3, 1), remuneracionDevengadaSac: 150_000 }), // la mejor del semestre
      remuneracion({ periodo: utc(2024, 5, 1), remuneracionDevengadaSac: 120_000 }),
    ];
  ...
  it('ignora meses no normales y habituales', () => {
    const remuneraciones = [
      remuneracion({ periodo: utc(2024, 1, 1), remuneracionDevengadaSac: 100_000 }),
      remuneracion({ periodo: utc(2024, 3, 1), remuneracionDevengadaSac: 500_000, esNormalYHabitual: false }),
    ];
  ...
  it('no cuenta meses del semestre posteriores a la fecha de egreso', () => {
    const remuneraciones = [
      remuneracion({ periodo: utc(2024, 1, 1), remuneracionDevengadaSac: 100_000 }),
      remuneracion({ periodo: utc(2024, 5, 1), remuneracionDevengadaSac: 999_999 }), // después del egreso
    ];
